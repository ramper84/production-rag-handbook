...
For years, the operational warning was that pre-filtering destroyed the HNSW index: the execution plan fell back to a sequential scan because the index did not know how to combine vector similarity with the structural filter. In modern pgvector versions (from 0.8), the situation has changed significantly thanks to **iterative scans over filtered HNSW**, which keep requesting candidates from the index until enough of them survive the `WHERE`. The behaviour is no longer optimal in every case but stops being catastrophic, and for selectivities below 20% the performance is reasonable. The programme adopts pre-filtering as the default strategy on the basis of this behaviour.

> *(Editor's note: corrected in this repo. The original said "from 0.7" and described the index as navigating the graph discarding candidates that fail the `WHERE`. Iterative index scans and the `hnsw.iterative_scan` setting arrived in **pgvector 0.8.0** — 0.7 brought `halfvec` and binary quantisation — and the mechanism is re-requesting candidates until enough survive the filter, not filter-aware traversal. s08-04 and [s10-06](s10-06-contextual-and-temporal-filtering.md) already had both right.)*
