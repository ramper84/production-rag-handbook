...
> *(Editor's note: the `document_type` column below is what Session 10 article 5, [Multi-index and routing](s10-05-multi-index-and-routing.md), calls "option A — one table with a discriminator column", and it revises this choice for the same system: once the corpus grew past budgets to transcripts and technical documentation, the metadata schemas diverged, and the article argues for one chunk table per family instead. The two-table documents/chunks relation and the typed-column/JSONB split described here survive the change; what does not is the assumption of a single chunk population. Checked against the code, this is settled rather than merely argued: `store/models.py` declares `documents` plus three chunk tables — `budget_chunks`, `transcript_chunks`, `technical_doc_chunks`.)*
