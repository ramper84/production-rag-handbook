...
> *(Editor's note: Session 10 article 4, [Query expansion and decomposition](s10-04-query-expansion-and-decomposition.md), is where the author builds this move out — and reaches a warmer verdict on decomposition for the same estimation system, on the grounds that a budget is built line item by line item. It also adds two things absent here: the distinction between expanding one intent and decomposing many, and the argument that the two must not fuse alike — expansion rewards consensus with RRF, decomposition needs round-robin coverage or the majority topic floods the context.)*
